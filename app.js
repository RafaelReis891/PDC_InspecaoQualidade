 ...
 *
 * Correções:
 * - Persistência do checklist selecionado
 * - Restauração após F5
 * - Restauração da inspeção em andamento
 * - Troca segura de checklist
 * - Nova inspeção limpa corretamente
...
const SELECTED_CHECKLIST_KEY =
    "controleQualidade_selectedChecklist";
...
function saveSelectedChecklist(checklistId) {
    ...
    try {
        ...
        console.warn(
            "Não foi possível salvar o checklist selecionado:",
            error
        );
    }
}
...
function getSelectedChecklist() {
    ...
    try {
        ...
        console.warn(
            "Não foi possível recuperar o checklist selecionado:",
            error
        );
...
function clearSelectedChecklist() {
    ...
    try {
        ...
        console.warn(
            "Não foi possível limpar o checklist selecionado:",
            error
        );
    }
}
...
async function initialize() {
    ...
    try {
        ...
        if (checklistSelect) {
            ...
            checklistSelect.addEventListener(
                "change",
                changeChecklist
            );
        }
        ...
        // ====================================================
        // PRIMEIRO:
        // TENTA RESTAURAR UMA INSPEÇÃO COMPLETA
        // ====================================================
        ...
        // NÃO EXISTE INSPEÇÃO
        //
        // RESTAURA O ÚLTIMO CHECKLIST
        // ====================================================

        const savedChecklistId =
...
async function loadChecklistOnly(
    selected,
    saveSelection = true
) {
...
function setChecklistVisibility(
    visible
) {
...
async function restoreSavedInspection(
    saved
) {
    ...
    try {
        ...
        if (!checklistId) {
            ...
            console.warn(
                "Checklist salvo não identificado."
            );
        ...
        if (!selected) {
            ...
            console.warn(
                "Checklist salvo não existe mais."
            );
...
// ============================================================
// LOCALIZAR CHECKLIST PELO NOME
// ============================================================

function findChecklistIdByName(
    name
) {
...
function validateChecklist() {
    ...
    if (incomplete !== -1) {
        ...
        showWarning(
            `A pergunta ${incomplete + 1} ainda não foi respondida.`
        );
...
async function newInspection() {
    ...
    // IMPORTANTE:
    // Nova inspeção também limpa o checklist salvo.
    clearSelectedChecklist();
